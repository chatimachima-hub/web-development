<x-layouts::app :title="__('User')">
    <flux:heading size="xl">Edite user</flux:heading>
    <flux:subheading class="mb-4">Form untuk mengedit user yang sudah ada di sistem</flux:subheading>
    ...
    <flux:card class="space-y-6">
        ...
        <form action="{{ route('user.update', $user) }}" method="POST" class="space-y-6">
            ...
            <flux:field>
                ...
                <flux:input name="name" require value="{{ old('name', $user->name) }}" />
                <flux:error name="name" />
            </flux:field>
            <flux:field>
                <flux:label>Email</flux:label>
                <flux:description>Alamat Email Pengguna.</flux:description>
                <flux:input name="email" type="email" require value="{{ old('email', $user->email) }}" />
